function gaussianElimination(augmentedMatrix) {
  for (let i = 0; i < augmentedMatrix.length; i++) { // O(n)
    let value = augmentedMatrix[i][i]; // O(1)
    for (let x = 0; x < augmentedMatrix.length + 1; x++) { // O(n+1) --> O(n^2 + n) 
      augmentedMatrix[i][x] = augmentedMatrix[i][x] / value;
    }
    for (let j = 0; j < augmentedMatrix.length; j++) { // O(n) --> O(n^2)
      let c = augmentedMatrix[j][i];
      for (let k = 0; k < augmentedMatrix.length + 1; k++) { // O(n+1) --> O(n^3 + n^2)
        if (j === i) {
          continue;
        }
