function maximumCommonDivider(number1, number2) {
  ...
  for (let i = 0; i < dividersNumber1.length; i++) { // O(n)
    for (let j = 0; j < dividersNumber2.length; j++) { // O(m) * O(n)--> O(m * n)
  ...
  // Regresamos el MCD, el cual fue el último en entrar al arreglo de divisores
  // en común
...
let result = maximumCommonDivider(15, 20);
console.log(result);